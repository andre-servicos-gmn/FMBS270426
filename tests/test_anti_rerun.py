...
async def test_recommend_allows_rerun_after_threshold():
    ...
    Sprint 2.0: with modelo_desejado='nenhum' (PROFILE mode), recommend
    delegates to consultoria_offer which calls the LLM but NOT the retriever.
    """
    from app.agent.nodes.recommend import recommend_node
    ...
    rec_mock = json.dumps({"messages": ["bloco 1", "bloco 2", "agendar?"]})

    with patch("app.adapters.openai_client.OpenAIClient.chat", new_callable=AsyncMock) as llm:
        llm.return_value = rec_mock
...
async def test_recommend_allows_rerun_with_substantive_new_message():
    ...
    state["messages"] = [
        HumanMessage(content="agora prefiro algo mais leve, com peso reduzido pra meu ombro")
    ]

    products = [{
        "id": "p1", "name": "Raquete Leve", "sport": "beach_tennis",
        "level": "intermediário", "price_cents": 60000, "stock": 5,
        "description": "desc", "similarity": 0.9, "external_id": "L1",
        "url": None, "image_url": None, "updated_at": None, "is_active": True,
        "weight_g": 320, "balance": "leve", "material": "carbono",
        "category": "raquete",
    }]
    rec_mock = json.dumps({"messages": ["*Raquete Leve*", "Posso reservar?"]})
